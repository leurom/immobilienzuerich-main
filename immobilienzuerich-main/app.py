...
def find_nearest_neighbor(lat, lng):
    ...
    # If there are no nodes within 1 km, set coefficient to 0
    """ if len(nodes_within_1km) == 0:
        coefficient = 0
    else:
        # Calculate a coefficient between 0 and 2 based on distance
        coefficient = (max_distance - nodes_within_1km.loc[0, 'distance']) / (max_distance - min_distance) * 2 """
    if number_of_nodes == 0:
        return 0

    # Normalisierung des Koeffizienten auf einen Bereich von 0 bis 2
    normalized_coefficient = (number_of_nodes - 1) / 1000
